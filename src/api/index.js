...
export const fetchSingleMovie = (movieId) => `https://api.themoviedb.org/3/movie/${movieId}?api_key=${API_KEY}&language=en-US`;
...
BASE_API.get(
    `movie/popular?api_key=${API_KEY}&release_date.lte=${date_to}&release_date.gte=${date_from}&with_genres=${genre}&sort_by=${choose}`
  );
...
BASE_API.get(
    `discover/movie?api_key=${API_KEY}&original_language=en-US&primary_release_date.gte=${date_to}&primary_release_date.lte=${date_from}&with_genres=${genre}&sort_by=${choose}`
  );
